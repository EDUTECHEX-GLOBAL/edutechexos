...
const isExtraWorkspaceChannel = (channelId: string) => isWorkspaceChannel(channelId) && channelId !== 'general';
...
const INITIAL_CHANNELS: Channel[] = CHANNELS.map((channel) => {
  ...
  const memberIds = INITIAL_MEMBERS
    .filter((member) => INITIAL_EXTRA_CHANNEL_BY_MEMBER[member.id] === channel.id)
...
export const useDashboardStore = create<DashboardState>()(
  ...
  aiSummary: {},
  setAiSummary: (channelId, summary) => set((state) => ({
    aiSummary: { ...state.aiSummary, [channelId]: summary }
  })),
  channels: INITIAL_CHANNELS,
  addChannel: (channel) => set((state) => {
    const memberIds = channel.id === 'general'
  ...
  addMember: (member) => set((state) => ({
    ...
    channels: state.channels.map((channel) => {
      ...
      return syncMemberCount({ ...channel, memberIds: withMember(channel.memberIds, member.id) });
    }),
    messages: {
      ...state.messages,
      [member.id]: []
  ...
  addMemberToChannel: (channelId, memberId) => set((state) => {
    if (channelId === 'general') {
      return {
        channels: state.channels.map((channel) => (
          channel.id === 'general'
            ? syncMemberCount({ ...channel, memberIds: withMember(channel.memberIds, memberId) })
            : channel
        )),
      };
    }
    ...
    if (isExtraWorkspaceChannel(channelId)) {
      return {
        channels: state.channels.map((channel) => {
          ...
          if (channel.id === 'general') {
            return syncMemberCount({ ...channel, memberIds: withMember(channel.memberIds, memberId) });
          }
          return syncMemberCount({
            ...channel,
            memberIds: channel.id === channelId
  ...
  removeMemberFromChannel: (channelId, memberId) => set((state) => {
    if (channelId === 'general') {
  ...
  setMemberWorkspaceChannel: (memberId, channelId) => set((state) => ({
    channels: state.channels.map((channel) => {
      if (!isWorkspaceChannel(channel.id)) return channel;
      if (channel.id === 'general') {
        return syncMemberCount({ ...channel, memberIds: withMember(channel.memberIds, memberId) });
