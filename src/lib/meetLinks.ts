...
export function getMeetingState(now = new Date()): MeetingState {
  ...
  if (day === 5)
    return { label: 'Friday meet',  link: FRIDAY_MEET_LINK,     message: 'Friday meeting link is active.' };
  if (day === 4 && hour >= 14)
    return { label: 'Thursday PM',  link: THURSDAY_PM_MEET_LINK, message: 'Thursday afternoon meeting link is active.' };
