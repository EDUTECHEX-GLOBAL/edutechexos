...
function isWeekend(ds: string) { const d = new Date(ds).getDay(); return d === 0 || d === 6; }
...
export default function MyActivityCalendar({ open, onClose }: Props) {
  ...
  useEffect(() => {
    const now = new Date();
    setViewYear(now.getFullYear()); setViewMonth(now.getMonth()); setMounted(true);
  }, []);
    ...
    try {
      ...
      fetch(`${apiBase}/api/login-history`, { headers: token ? { Authorization: `Bearer ${token}` } : {} })
        ...
        .then(data => {
          ...
          if (!stored.length) {
            const gen: string[] = []; const t = new Date();
            for (let i = 0; i < 30; i++) { const d = new Date(); d.setDate(t.getDate() - i); if (Math.random() < (d.getDay() === 0 || d.getDay() === 6 ? 0.1 : 0.85)) gen.push(d.toISOString().split('T')[0]); }
            const ts = t.toISOString().split('T')[0]; if (!gen.includes(ts)) gen.push(ts);
  ...
  const workdaysElapsed = Array.from({ length: daysInMonth }, (_, i) => { const ds = toDateStr(viewYear, viewMonth, i + 1); return ds <= todayStr && !isWeekend(ds); }).filter(Boolean).length;
  ...
  return (
    <div ref={overlayRef} onClick={e => { if (e.target === overlayRef.current) onClose(); }}
      ...
      <div className="w-full max-w-[340px] rounded-2xl overflow-hidden shadow-2xl animate-in slide-in-from-bottom-4 duration-300"
        style={{ background: '#111827', border: '1px solid rgba(255,255,255,0.07)' }}>

        {/* ── Header ── */}
        <div className="flex items-center justify-between px-5 pt-5 pb-4">
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 rounded-xl flex items-center justify-center text-xs font-black text-white shadow-md shrink-0"
        ...
        <div className="mx-4 mb-4 grid grid-cols-3 gap-2">
          {[
            { icon: <CalendarDays size={14} />, value: thisMonthLogins, label: 'This month',  color: '#34d399', bg: 'rgba(52,211,153,0.08)'  },
            { icon: <Flame        size={14} />, value: streak,          label: 'Day streak',  color: '#fb923c', bg: 'rgba(251,146,60,0.08)'   },
            { icon: <TrendingUp   size={14} />, value: `${loginRate}%`, label: 'Active rate', color: '#38bdf8', bg: 'rgba(56,189,248,0.08)'   },
          ].map((s, i) => (
            <div key={i} className="flex flex-col items-center gap-1.5 rounded-xl py-3 px-2" style={{ background: s.bg }}>
              <span style={{ color: s.color }}>{s.icon}</span>
              <span className="text-xl font-black tabular-nums text-white" style={{ color: s.color }}>{s.value}</span>
              <span className="text-[9px] font-semibold text-white/35 text-center leading-tight">{s.label}</span>
            </div>
          ))}
        </div>
        ...
        <div className="px-4 pb-4">
          ...
          <div className="grid grid-cols-7 mb-2">
            {WEEKDAYS.map((d, i) => (
              <div key={i} className="text-center text-[9px] font-bold text-white/20 py-1">{d}</div>
            ))}
          </div>
            ...
            {Array.from({ length: daysInMonth }, (_, i) => {
              ...
              if (!isFuture) {
                if (didLogin) {
                  ...
                  cellStyle = { background: 'rgba(244,63,94,0.08)', border: '1px solid rgba(244,63,94,0.15)' };
              ...
              if (isToday) {
                cellStyle = { ...cellStyle, outline: `2px solid ${userColor}`, outlineOffset: '2px' };
        ...
        {/* ── Progress bar + legend ── */}
        <div className="mx-4 mb-4 rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-semibold text-white/30">Attendance this month</span>
            <span className="text-[10px] font-bold tabular-nums" style={{ color: userColor }}>{thisMonthLogins}/{workdaysElapsed} days</span>
          </div>
          <div className="h-1.5 w-full rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.06)' }}>
            <div className="h-full rounded-full transition-all duration-700" style={{ width: `${Math.min(loginRate, 100)}%`, background: userColor }} />
          </div>
          <div className="flex items-center gap-4 mt-2.5">
            ...
            <span className="ml-auto text-[9px] font-semibold text-white/20 tabular-nums">{loginDates.length} total</span>
          </div>
        </div>
