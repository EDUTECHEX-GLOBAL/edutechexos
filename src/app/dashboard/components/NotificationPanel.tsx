...
import { X, MessageSquare, Smile, Pin, CheckSquare, Bell, BellOff, AtSign, ChevronDown, ChevronUp, Clock, Hash } from 'lucide-react';
...
const TYPE_CONFIG = {
  reply:    { icon: MessageSquare, color: '#3E4A89', bg: '#eef2ff', label: 'Reply',    gradient: 'linear-gradient(135deg,#3E4A89,#9BA6D3)' },
  reaction: { icon: Smile,         color: '#f59e0b', bg: '#fffbeb', label: 'Reaction', gradient: 'linear-gradient(135deg,#f59e0b,#fbbf24)' },
  pin:      { icon: Pin,           color: '#8b5cf6', bg: '#f5f3ff', label: 'Pinned',   gradient: 'linear-gradient(135deg,#8b5cf6,#a78bfa)' },
  task:     { icon: CheckSquare,   color: '#10b981', bg: '#ecfdf5', label: 'Task',     gradient: 'linear-gradient(135deg,#10b981,#34d399)' },
  mention:  { icon: AtSign,        color: '#ef4444', bg: '#fef2f2', label: 'Mention',  gradient: 'linear-gradient(135deg,#ef4444,#f87171)' },
};
    ...
    <>
      ...
      >
        ...
        <div className="shrink-0 px-5 pt-5 pb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl"
                style={{ background: 'linear-gradient(135deg,#3E4A89,#9BA6D3)', boxShadow: '0 4px 12px rgba(62,74,137,0.50)' }}>
              ...
              <div>
                ...
                <p className="text-[11px] font-bold uppercase tracking-widest" style={{ color: 'rgba(165,180,252,0.70)' }}>
          ...
          {/* Shimmer divider */}
          <div className="mt-4 h-px" style={{ background: 'linear-gradient(90deg,transparent,rgba(165,180,252,0.35),transparent)' }} />
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-4 space-y-2" style={{ scrollbarWidth: 'none' }}>
          {notifications.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-14 gap-4">
              <div className="h-14 w-14 rounded-2xl flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.10)' }}>
                <BellOff size={24} style={{ color: 'rgba(165,180,252,0.40)' }} />
              </div>
              <div className="text-center">
                <p className="text-sm font-black text-white/60">All caught up</p>
                <p className="text-xs mt-0.5" style={{ color: 'rgba(165,180,252,0.45)' }}>No notifications yet</p>
              </div>
            </div>
              ...
              <motion.div
                ...
                style={{
                  background: isExpanded
                    ...
                    : notif.read ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.08)',
                  border: isExpanded
                    ? `1px solid ${cfg.color}55`
                    : notif.read ? '1px solid rgba(255,255,255,0.06)' : `1px solid ${cfg.color}33`,
              ...
              >
                ...
                <div className="flex items-center gap-3 px-3.5 py-3">
                  ...
                  <div className="relative shrink-0">
                    <div className="h-9 w-9 rounded-xl flex items-center justify-center text-[11px] font-black text-white shadow-sm"
                      style={{ background: notif.actorColor }}>
                      {notif.actorInitials}
                    </div>
                    <div className="absolute -bottom-1 -right-1 h-4.5 w-4.5 rounded-md flex items-center justify-center border-2"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {/* Type pill */}
                      <span className="shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-black uppercase tracking-widest"
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <span className="text-[9px] font-bold" style={{ color: 'rgba(165,180,252,0.55)' }}>
                      {formatRelativeTime(notif.timestamp)}
                    </span>
                    {!notif.read && (
                      <div className="h-1.5 w-1.5 rounded-full animate-pulse" style={{ background: cfg.color }} />
                    )}
                    {isExpanded
                ...
                <AnimatePresence initial={false}>
                  {isExpanded && (
                    ...
                    >
                      ...
                      <div className="mx-3.5 h-px" style={{ background: `linear-gradient(90deg,transparent,${cfg.color}55,transparent)` }} />

                      <div className="px-3.5 py-3 space-y-3">
                        {/* Full message highlighted */}
                        <div className="rounded-xl px-3 py-2.5" style={{ background: `${cfg.color}14`, border: `1px solid ${cfg.color}30` }}>
                          <p className="text-[13px] leading-relaxed font-medium" style={{ color: 'rgba(255,255,255,0.88)' }}>
                            {notif.message}
                          </p>
                        </div>
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                          <div className="flex items-center gap-1.5 rounded-lg px-2 py-1" style={{ background: 'rgba(255,255,255,0.07)' }}>
                        ...
                        <div className="flex items-center justify-between">
                          <button
                            onClick={(e) => { e.stopPropagation(); dismiss(notif.id); setExpandedId(null); }}
                          ...
                          <div className="flex items-center gap-1.5 rounded-full px-2.5 py-1"
                            style={{ background: cfg.gradient, boxShadow: `0 2px 8px ${cfg.color}44` }}>
