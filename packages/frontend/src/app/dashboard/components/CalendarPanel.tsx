...
function extractDate(text: string): { date: Date | null; raw: string } {
  ...
  if (m) {
    ...
    if (!isNaN(d.getTime())) return { date: d, raw: m[1] };
  }

  m = text.match(new RegExp(`(\\d{1,2})(?:st|nd|rd|th)?\\s+(${MO})`, 'i'));
  if (m) {
    const d = new Date(`${m[2]} ${m[1]} ${now.getFullYear()}`);
    if (!isNaN(d.getTime())) return { date: d, raw: m[0] };
  }
...
function extractTaskTitle(text: string): string {
  let t = text.replace(/\n+/g, ' ').replace(/\s+/g, ' ').trim();
  const m = t.match(
    /(?:deadline[:\s]+|due\s+(?:by\s+|date[:\s]+)|submit\s+by[:\s]+|complete\s+by[:\s]+|action\s+item[:\s]+)(.{4,80})/i
  );
  if (m) t = m[1].trim();
...
function parseDT(s: string): Date | null {
  try {
    ...
    const m = s.match(/(\d{4}-\d{2}-\d{2})\s+at\s+(\d{2}:\d{2})/);
...
const isUpcoming = (s: string) => {
  const d = parseDT(s);
  return d ? d.getTime() > Date.now() - 3600000 : false;
};
const fmtTime = (s: string) =>
  parseDT(s)?.toLocaleString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true }) ?? s;
const fmtDate = (s: string) =>
  parseDT(s)?.toLocaleString('en-IN', { weekday: 'long', month: 'long', day: 'numeric' }) ?? s;
function dateBlock(s: string) {
  const d = parseDT(s);
  if (!d) return { mo: '—', dy: '—' };
  return {
    mo: d.toLocaleString('en-IN', { month: 'short' }).toUpperCase(),
    dy: String(d.getDate()),
  };
}
function dateBlockMs(ms: number | null) {
  if (!ms) return { mo: '?', dy: '?' };
  const d = new Date(ms);
  return {
    mo: d.toLocaleString('en-IN', { month: 'short' }).toUpperCase(),
...
function deadlineFmtDate(ms: number | null) {
...
function getCurrentUser() {
  ...
  try {
    return JSON.parse(localStorage.getItem('edutechex_token') ?? '')?.user ?? null;
  } catch {
    return null;
  }
}
function canJoin(
  m: { sender: string; people: string },
...
export default function CalendarPanel({ onClose }: CalendarPanelProps) {
  ...
  const allMeetings = useMemo(() => {
    ...
    Object.entries(messages).forEach(([ch, msgs]) =>
      (msgs as Message[]).forEach((msg) => {
        const p = parseScheduledMeet(msg.text ?? '');
        if (p)
          r.push({
            id: msg.id,
            channel: ch,
    ...
    return r.sort(
      (a, b) => (parseDT(a.timeStr)?.getTime() ?? 0) - (parseDT(b.timeStr)?.getTime() ?? 0)
    );
  }, [messages]);
                ...
                {upcoming.length === 0 ? (
                  <div className="flex flex-col items-center gap-3 py-14 text-center">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs text-[#9BA6D3]">
                        Ask the AI assistant to schedule a meeting — it will appear here.
                      </p>
                    </div>
                  </div>
                ) : (
                                ...
                                <div className="flex flex-1 items-center gap-1">
                                  ...
                                  <button
                                    ...
                                    onClick={async () => {
                                      ...
                                      try {
                                        const tk = JSON.parse(
                                          localStorage.getItem('edutechex_token') || '{}'
                                        )?.token;
                                        await fetch(
                                          `${API_BASE}/api/meeting-access/${m.id}/grant`,
                                          {
                                            method: 'PATCH',
                                            headers: {
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] font-semibold text-[#7C859E]">
                      {scanning
                        ...
                        : scanAge !== null
                          ? `Last scanned ${scanAge < 2 ? 'just now' : `${scanAge}m ago`} · ${deadlines.length} item${deadlines.length !== 1 ? 's' : ''} found`
                          : 'Not yet scanned'}
                    </span>
                  </div>
                  <button
                  ...
                  {futureDeadlines.length === 0 && !scanning ? (
                    <div className="flex flex-col items-center gap-3 py-12 text-center">
                      ...
                      <div>
                        ...
                        <p className="mt-1 max-w-xs text-xs text-[#9BA6D3] leading-relaxed">
                          Mention words like <strong>deadline</strong>, <strong>due by</strong>,{' '}
                          <strong>submit by</strong>, or <strong>complete by</strong> in any channel
                          message and they will show up here automatically.
                        </p>
                      </div>
                    </div>
                  ) : (
                    <>
                      ...
                      {futureDeadlines.length > 0 && (
                        <>
                          <p className="mb-1 text-[9px] font-black uppercase tracking-widest text-[#9BA6D3]">
                            Upcoming · {futureDeadlines.length}
                          </p>
                          {futureDeadlines.map((d) => {
                            ...
                            return (
                              <div
                                key={d.id}
                                className="flex gap-3 rounded-2xl border border-[rgba(62,74,137,0.08)] bg-white p-3.5 shadow-sm"
                              >
                                {/* Date block */}
                                <div
                                  className={`flex w-11 shrink-0 flex-col items-center justify-center rounded-xl py-1.5 ${urgencyColor(d.dateMs)}`}
                                >
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-start justify-between gap-2">
                                    <p className="text-xs font-black text-[#1E2636] leading-snug">
                                      {d.task}
                                    </p>
                                    {d.dateMs && (
                                      <span
                                        className={`shrink-0 rounded-full px-2 py-0.5 text-[9px] font-black ${urgencyBadge(d.dateMs)}`}
                                      >
                                  ...
                                  <div className="mt-1 flex items-center gap-3 text-[10px] text-[#9BA6D3]">
                                    ...
                                    {d.dateMs && (
                                      <span className="flex items-center gap-1">
                                        <Clock size={9} />
                                        {deadlineFmtDate(d.dateMs)}
                                      </span>
                                    )}
                                  </div>
                                  <p className="mt-1.5 rounded-lg bg-[#F7F6F2] px-2.5 py-1.5 text-[10px] leading-relaxed text-[#4A5578] italic">
                                    &quot;{d.snippet}&quot;
                                  </p>
                                  <div className="mt-2 flex items-center gap-2">
                                    <a
                                      href={gcalEventUrl(d.task, d.dateMs)}
                                      target="_blank"
                                      rel="noreferrer"
                      ...
                      {pastDeadlines.length > 0 && (
                        <div className="mt-4">
                          <p className="mb-1 text-[9px] font-black uppercase tracking-widest text-[#C4CAE0]">
                            Past deadlines · {pastDeadlines.length}
                          </p>
                          {pastDeadlines.map((d) => (
                            <div
                              key={d.id}
                              className="mb-1.5 flex items-center gap-3 rounded-xl border border-[rgba(62,74,137,0.06)] bg-white px-3 py-2 opacity-50"
                            >
                              <div className="flex w-9 shrink-0 flex-col items-center justify-center rounded-lg bg-slate-100 py-1">
                                <span className="text-[7px] font-black uppercase text-slate-400">
                                  {dateBlockMs(d.dateMs).mo}
                                </span>
                                <span className="text-sm font-black leading-none text-slate-400">
                                  {dateBlockMs(d.dateMs).dy}
                                </span>
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className="truncate text-xs font-bold text-[#7C859E] line-through">
                                  {d.task}
                                </p>
              ...
              >
                {past.length === 0 ? (
                  <div className="flex flex-col items-center gap-3 py-14 text-center">
                    ...
                    <p className="text-sm font-bold text-[#9BA6D3]">No past meetings yet</p>
                  </div>
                ) : (
                  <div className="space-y-1.5">
                    {past.map((m) => {
                      ...
                      return (
                        <div
                          key={m.id}
                          className="flex items-center gap-3 rounded-xl border border-[rgba(62,74,137,0.07)] bg-white px-4 py-3 opacity-65 hover:opacity-100 transition-opacity"
                        >
                          <div className="flex w-10 shrink-0 flex-col items-center justify-center rounded-lg bg-[#F0F1F7] py-1">
                            <span className="text-[7px] font-black uppercase text-[#9BA6D3]">
                              {mo}
                            </span>
                            <span className="text-base font-black leading-none text-[#7C859E]">
                              {dy}
                            </span>
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-[#9BA6D3]">
                              {fmtTime(m.timeStr)}
                              {cs.length > 0 &&
                                ` · ${cs.length} attendee${cs.length !== 1 ? 's' : ''}`}
                            </p>
                          </div>
                          <span className="shrink-0 text-[10px] font-bold text-[#B0B8D1]">
                            {m.sender}
                          </span>
                        </div>
                      );
