...
export default function LoginForm({
  ...
  // ── Google Sign-In: handle the redirect back from Google ────────────────────
  useEffect(() => {
    const googleAuthResult = searchParams?.get('googleAuth');
    if (googleAuthResult === 'not-registered') {
      toast.error('That Google account isn\'t registered yet. Ask an admin to invite you first.');
    } else if (googleAuthResult === 'wrong-portal') {
      toast.error('Use the correct login page for this account (Admin vs. regular user).');
    } else if (googleAuthResult === 'error') {
      toast.error('Google Sign-In failed. Please try again.');
    }

    // The backend redirects back with the token in the URL fragment (#...),
    // not a query param, so it never ends up in server logs or browser history search.
    const hash = window.location.hash;
    if (hash.startsWith('#googleToken=')) {
      try {
  ...
  async function signInWithGoogle() {
    try {
      const res = await fetch(`${API_BASE}/api/auth/google/url?mode=${authMode}`);
      const data = await res.json();
      if (!res.ok || !data.success) {
        toast.error(data.error || 'Google Sign-In is not available right now.');
        return;
      }
  ...
  async function finishLogin(
    ...
    if (authMode === 'user' && loginAccount.role === 'Admin') {
      ...
      // Best-effort: the session cookie is a convenience (inline file loads);
      // the localStorage token is the primary auth. A failure here must NOT
      // block the redirect below, or the user is stranded on the login page.
      try {
    ...
    localStorage.setItem('edutechex_token', JSON.stringify({ user: loginAccount, token }));
    startOrResumeSession();
    // Best-effort session cookie (see admin path above) — never let it block the redirect.
    try {
      await fetch('/api/auth/session', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
        ...
        <div className="pt-4">
          ...
          <div className="flex items-center gap-3 my-4">
            <div className="h-px flex-1 bg-[#0A1128]/10" />
            <span className="text-[10px] font-bold uppercase tracking-widest text-[#0A1128]/30">or</span>
            <div className="h-px flex-1 bg-[#0A1128]/10" />
          </div>
          ...
          <button
            ...
            disabled={isLoading}
            className="flex w-full items-center justify-center gap-2.5 rounded-sm border border-[#0A1128]/12 bg-white py-4 text-sm font-bold text-[#0A1128] transition-all hover:bg-[#0A1128]/[0.03] disabled:opacity-50 disabled:cursor-not-allowed"
          >
