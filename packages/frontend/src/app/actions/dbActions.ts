...
// ── Email helper — calls backend internal API ──
export async function sendBrevoEmail(opts: {
  ...
  const backendUrl = process.env.BACKEND_URL || process.env.NEXT_PUBLIC_API_URL || 'http://localhost:10002';
  const secret = process.env.INTERNAL_API_SECRET;
  if (!secret) {
    return { success: false, error: 'INTERNAL_API_SECRET not configured' };
  }
  try {
    const res = await fetch(`${backendUrl}/api/internal/send-email`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-internal-key': secret },
      body: JSON.stringify({ to: opts.to, subject: opts.subject, htmlContent: opts.htmlContent, bcc: opts.bcc }),
    });
    const data = await res.json();
    if (data.success) {
      console.log(`[Mail] Backend API OK — "${opts.subject}" to ${opts.to.length} recipient(s)`);
      return { success: true };
    }
    console.error('[Mail] Backend API error:', data);
    return { success: false, error: data.error || 'Backend email API failed' };
  } catch (e) {
    console.error('[Mail] Backend API call failed:', e);
