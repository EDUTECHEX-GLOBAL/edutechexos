  ...
  async function buildSummary(forDate) {
    ...
    if (!gateMs) return null; // never logged into the web app that day → count nothing
    ...
    for (const wb of webBuckets) {
      ...
      for (const ev of webEvents) {
        ...
        try {
          ...
          if (!domainSecs[title]) domainSecs[title] = { secs: 0, domain };
          domainSecs[title].secs += dur;
        } catch { /* invalid URL */ }
      }
    }
  ...
  (async () => {
    ...
    console.log(`[agent] Started. Syncing every ${INTERVAL} minute(s); live app ping every 15s.`);
